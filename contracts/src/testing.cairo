...
mod tests {
    ...
    use emojiman::models::{
        player_id, player_address, click_power, score, inventory, PlayerID, PlayerAddress,
        ClickPower, Score, Inventory
    };
    ...
    fn spawn_test() {
        ...
        //switch to caller address 2
        let caller_2 = starknet::contract_address_const::<0x22>();
        starknet::testing::set_contract_address(caller_2);
        actions_.spawn();

        // Get player 2 ID
        let player_id_2: u8 = get!(world, caller_2, (PlayerID)).id;
        player_id_2.print();
        assert(2 == player_id_2, 'incorrect id');

        //Get player ClickPower
        let click_power = get!(world, player_id_2, (ClickPower)).value;
        assert(click_power == 10, 'incorrect click_power');

        // Get player Score 
        let score = get!(world, player_id_2, (Score)).value;
        assert(score == 50, 'starting score should be 50');
    }
